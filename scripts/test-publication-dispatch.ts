...
} from '../src/server/approval/snapshot.ts'
import {
...
test('STEP 15E.2 Dispatch Boundary Test Suite', async (t) => {
  ...
  await t.test('2. Baseline: platform.publish Tool is unavailable and requires approval', () => {
    ...
    assert.deepEqual(
      tool.risk,
      ['write', 'external'],
      'platform.publish risks must be write and external',
    )
  })

  await t.test(
    '3. Baseline: prepareToolExecution for platform.publish fails safely as unavailable',
    () => {
      ...
      if (!prep.ok) {
        assert.ok(
          prep.error.code === 'capability_denied' ||
            prep.error.code === 'not_configured' ||
            prep.error.code === 'tool_disabled',
  ...
  // ==========================================
  await t.test(
    '4. Initiation: Critic review pass alone does not publish or create Approval Request',
    async () => {
      const env = await setupTestEnvironment()
      const posts = await queryAll<{ status: string }>(
        env.db,
        'SELECT status FROM post WHERE id = ?',
        [env.postId],
      )
      assert.equal(posts[0]?.status, 'draft', 'Post must remain draft')
      const approvals = await queryAll(env.db, 'SELECT * FROM approval WHERE subject_id = ?', [
        env.postId,
      ])
  ...
  await t.test(
    '5. Initiation: Editorial approval alone does not publish or create Approval Request',
    async () => {
      const env = await setupTestEnvironment()
  ...
  await t.test(
    '6. Initiation: Publication intent preparation alone does not publish or dispatch',
    async () => {
      const env = await setupTestEnvironment()
      const post = await getPostDetail(env.db, env.workspaceId, env.postId)
      assert.equal(post?.status, 'draft')
  ...
  await t.test(
    '7. Initiation: Explicit user publish request creates Approval Request',
    async () => {
      const env = await setupTestEnvironment()
  ...
  // ==========================================
  await t.test(
    '8. Authority: Request accepts only workspaceId & postId; ignores client forged copies',
    async () => {
      const env = await setupTestEnvironment()
      const result = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
      ...
      } as unknown as Parameters<typeof requestPublicationDispatch>[1])
      ...
      assert.equal(
        snapshot.accountId,
        env.accountId,
        'Snapshot must use authoritative accountId from DB',
      )
      assert.equal(
        snapshot.contentVariantId,
        env.variantId,
        'Snapshot must use authoritative variantId from DB',
      )
    },
  )
  ...
  // ==========================================
  await t.test(
    '10. Pre-Approval: Revoked editorial approval rejects publish request before creating Approval Request',
    async () => {
      const env = await setupTestEnvironment()
  ...
  await t.test(
    '11. Pre-Approval: Paused target account rejects publish request before creating Approval Request',
    async () => {
      const env = await setupTestEnvironment()
  ...
  await t.test(
    '12. Pre-Approval: Archived campaign rejects publish request before creating Approval Request',
    async () => {
      const env = await setupTestEnvironment()
      await execute(env.db, "UPDATE campaign SET status = 'archived' WHERE id = ?", [
        env.campaignId,
      ])
  ...
  await t.test(
    '13. Pre-Approval: Disconnected campaign account rejects publish request before creating Approval Request',
    async () => {
      const env = await setupTestEnvironment()
      await execute(
        env.db,
        'DELETE FROM campaign_account WHERE campaign_id = ? AND account_id = ?',
        [env.campaignId, env.accountId],
      )
  ...
  // ==========================================
  await t.test(
    '14. Policy: actionKey content.publish is defined with category content and defaultMode review',
    () => {
  ...
  await t.test(
    '15. Policy: Permissive workspace policy AUTO is elevated to REVIEW by hard minimum mode',
    async () => {
      const env = await setupTestEnvironment()
      ...
      assert.equal(
        res.request?.policySource,
        'tool_requirement',
        'Source must reflect tool_requirement elevation',
      )
    },
  )

  await t.test(
    '16. Policy: BLOCKED policy returns status blocked, emits event, and creates no Approval Request',
    async () => {
      const env = await setupTestEnvironment()
  ...
  // ==========================================
  await t.test(
    '17. Approval Creation: Correct risk tags, human summary, snapshot and fingerprint',
    async () => {
      const env = await setupTestEnvironment()
      const res = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
      ...
      assert.equal(
        verifySnapshotIntegrity('content.publish', req.snapshotJson, req.fingerprint),
        true,
      )
    },
  )
  ...
  await t.test(
    '18. Idempotency: Duplicate/double-click request reuses existing pending Approval Request',
    async () => {
      const env = await setupTestEnvironment()
  ...
  // ==========================================
  await t.test(
    '20. Post State: Derived dispatchStatus is awaiting_approval when pending approval exists',
    async () => {
      const env = await setupTestEnvironment()
  ...
  // ==========================================
  await t.test(
    '21. Decision: Rejecting Approval Request leaves post unpublished and emits event',
    async () => {
      const env = await setupTestEnvironment()
      const reqRes = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
  ...
  await t.test(
    '22. Decision: Attempting dispatch on non-approved request fails safely',
    async () => {
      const env = await setupTestEnvironment()
      const reqRes = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
  ...
  // ==========================================
  await t.test(
    '23. Revalidation: Revoking editorial approval after approval fails dispatch safely',
    async () => {
      const env = await setupTestEnvironment()
      const reqRes = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
  ...
  await t.test(
    '24. Revalidation: Pausing account after approval fails dispatch safely',
    async () => {
      const env = await setupTestEnvironment()
      const reqRes = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
  ...
  await t.test(
    '25. Revalidation: Tampered snapshot integrity fails dispatch with integrity error',
    async () => {
      const env = await setupTestEnvironment()
      const reqRes = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
  ...
  // ==========================================
  await t.test(
    '26. Dispatch Boundary: Approved eligible dispatch fails safely as not_configured',
    async () => {
      const env = await setupTestEnvironment()
      const reqRes = await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
  ...
  await t.test(
    '27. Parity: derivePostState and validatePublicationEligibility agree on awaiting_approval',
    async () => {
      const env = await setupTestEnvironment()
      await requestPublicationDispatch(env.db, {
        workspaceId: env.workspaceId,
