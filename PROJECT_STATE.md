...
| X OAuth Token Refresh & Disconnect Lifecycle (Step 15E.3C.3) | src/server/platforms/adapters/x/oauth/ + src/features/accounts/ | Complete X OAuth 2.0 credential lifecycle; server-controlled refresh threshold (60s); distributed optimistic lease via D1 `platform_credential.refresh_locked_until` column (migration 0025); atomic credential vault rotation; transparent pre-dispatch refresh in `dispatchApprovedPublication`; safe `reconnect_required` normalization on invalid_grant/missing refresh tokens; authoritative local disconnect (revokes active credentials + sets connection to `disconnected`) before best-effort provider token revocation (`POST /2/oauth2/revoke`); clean UI with confirmation guard for Disconnect X |
