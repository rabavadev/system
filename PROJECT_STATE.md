...
| X OAuth 2.0 PKCE Account Flow (Step 15E.3C.2) | src/server/platforms/adapters/x/oauth/ + src/features/accounts/ | Complete user-facing OAuth 2.0 PKCE Authorization Code flow for X; PKCE S256 code challenge; encrypted short-lived HttpOnly transaction state cookie (`PLATFORM_OAUTH_STATE_KEK_V1`); server-authoritative redirect URI and scope validation; authenticated identity preflight (`GET /2/users/me`) before vault persistence; atomic credential replacement; zero token leakage; Clean UI with Connect/Reconnect actions on Account Detail page |
| X OAuth Token Refresh & Disconnect Lifecycle (Step 15E.3C.3 / Hardening 15E.3C.3.3) | src/server/platforms/adapters/x/oauth/ + src/features/accounts/ | Complete X OAuth 2.0 credential lifecycle; server-controlled refresh threshold (60s); distributed optimistic lease via D1 `platform_credential.refresh_locked_until` (migration 0025) and database-authoritative fencing via `refresh_claim_id` (migration 0026); owner-fenced **in-place** atomic vault rotation (`rotateOAuthCredentialFenced` — single conditional UPDATE on exact `id` + `refresh_claim_id` + `refresh_locked_until >= now` + `revoked_at IS NULL`, clears lease atomically, preserves refresh token material when provider omits replacement, cannot resurrect a disconnected account); transparent pre-dispatch refresh in `dispatchApprovedPublication`; safe `reconnect_required` normalization on invalid_grant/missing refresh tokens; authoritative local disconnect (revokes active credentials + sets connection to `disconnected`) before best-effort provider token revocation (`POST /2/oauth2/revoke`); clean UI with confirmation guard for Disconnect X |
...
- **STEP 15E.3C.2 / HARDENING 15E.3C.2.1 — X OAuth 2.0 PKCE Account Connection Flow & Client Contract Hardening**: Implemented user-facing OAuth 2.0 PKCE Authorization Code flow for X with PKCE S256 code challenge; explicit public and confidential client support with HTTP Basic client authentication (`Authorization: Basic base64(client_id:client_secret)`) and Worker secret resolution (`X_OAUTH_CLIENT_SECRET`); encrypted short-lived HttpOnly transaction state cookie (`PLATFORM_OAUTH_STATE_KEK_V1`, AES-256-GCM, SameSite=Lax, Path=/); server-authoritative redirect URI and scope validation; authenticated identity preflight (`GET /2/users/me`) before vault persistence; atomic credential replacement via D1 batch; zero token leakage; Clean UI with Connect/Reconnect actions on Account Detail page; 115 comprehensive test scenarios (Public/Confidential Client support: YES; Client Secret storage: Worker secret only; OAuth browser E2E: NOT TESTED; Refresh lifecycle: NOT IMPLEMENTED; Provider token revocation: NOT IMPLEMENTED).
- **STEP 15E.3C.3 / HARDENING 15E.3C.3.1 — X OAuth Token Refresh & Secure Disconnect Lifecycle**: Implemented complete credential lifecycle for X OAuth 2.0: server-controlled pre-use token refresh threshold (60s) before publication dispatch; distributed optimistic concurrency claim lease using D1-authoritative `refresh_locked_until` (migration `0025_credential_refresh_claim.sql`); atomic encrypted vault rotation via `storeOAuthCredential`; graceful `reconnect_required` / `connection_inactive` error normalization without post corruption or leakages; explicit Disconnect X server function (`disconnectXAccountFn`) and UI with confirmation guard; strict local-first authoritative revocation with subsequent best-effort provider revocation (`POST /2/oauth2/revoke` for access and refresh tokens); zero token leaks across DTOs, logs, audit trails, and events (OAuth authorization = implemented; encrypted vault = implemented; pre-use refresh = implemented; refresh concurrency mechanism = D1 optimistic lease via `refresh_locked_until`; Disconnect X = implemented; provider revocation = implemented; background refresh = NOT IMPLEMENTED; real refresh = NOT TESTED; real revoke = NOT TESTED; browser disconnect = NOT TESTED; Publisher = DISABLED).
- **HARDENING 15E.3C.3.2 — Stale Refresh Owner Fencing**: Implemented database-authoritative fencing for distributed OAuth token refresh. Adds `refresh_claim_id` column to `platform_credential` (migration `0026_credential_refresh_fencing.sql`); conditional lease claim generates high-entropy claim ID and updates `refresh_locked_until` + `refresh_claim_id`; vault persistence (`storeOAuthCredential`) verifies `expectedRefreshClaimId` matches active unrevoked row; conditional release (`releaseRefreshLease`) clears claim only when matching `claimId`; stale isolates whose lease expired or was reclaimed by another worker are strictly rejected with `stale_refresh_claim` and cannot overwrite newer active credentials or clear newer leases; 86 comprehensive test scenarios pass.
- **HARDENING 15E.3C.3.3 — Fenced In-Place OAuth Token Rotation**: Closed the stale-refresh persistence race caused by concurrent disconnect + in-flight refresh. Replaced revoke-and-insert rotation with a single conditional in-place `UPDATE platform_credential SET … WHERE id = ? AND workspace_id = ? AND account_id = ? AND revoked_at IS NULL AND refresh_claim_id = ? AND refresh_locked_until >= ?` (`rotateOAuthCredentialFenced` in `src/server/platforms/credentials/store.ts`). The UPDATE simultaneously proves exact credential identity, active state, exact claim ownership, and valid lease; clears `refresh_claim_id` + `refresh_locked_until` atomically; preserves existing refresh token material when provider omits a replacement (`COALESCE`). If the credential was revoked by disconnect before the UPDATE runs, `changes === 0` → `stale_refresh_claim` with no new row inserted and no `platform_connection` touched — disconnected accounts cannot be resurrected. Initial OAuth connect/reconnect continues to use `storeOAuthCredential` with replacement/history semantics. `claimRefreshLease` and `releaseRefreshLease` updated to accept `credentialId` and apply conditions on the exact row. `RotateOAuthCredentialFencedInput` / `RotateOAuthCredentialFencedResult` types added. 92 comprehensive test scenarios pass (tests 86–91 cover: in-place ID preservation, provider-omitted refresh token COALESCE, expired-lease rejection, wrong credentialId rejection, claim mismatch, and fenced failure halting publication dispatch with zero tweet/users calls).
...
Campaign Content Item (draft)
  ...
  → Provider Confirmed 201 Response (post: status = 'published', external_id = provider_id, published_at = timestamp)
